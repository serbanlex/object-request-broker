...
class MessageServer {
    ...
    private Object callMethodIssuedBy(ServerMessage request) throws ClassNotFoundException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        ...
        if (request.parameters == null) {
            method = proxiedClass.getMethod(request.methodName);
            result = method.invoke(proxiedInstance);
        } else {
            fixBigDecimalParams(request);
            method = proxiedClass.getMethod(request.methodName, getParameterTypes(request.parameters)); // get the method with the given name and parameters
    ...
    private void fixBigDecimalParams(ServerMessage request) {
        for (int i = 0; i < request.parameters.length; i++) {
            if (request.parameters[i] instanceof BigDecimal){
                request.parameters[i] = ((BigDecimal) request.parameters[i]).doubleValue();
            }
        }
    }
